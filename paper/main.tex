...
where $\lvert C \rvert \leq \frac{\lvert P \rvert}{x}$ for some $x$, but this is not fundamental and we only consider a single $C$ at a time.
...
A protocol in which no information (random bits don't count) is communicated is automatically secure,
although of course it can only correctly implement trivial functionalities.
On the other hand, if the adversary can efficiently deduce a unique possible honest input upon observing the corrupt output,
then the security is vacuous; any correct implementation of that functionality is secure regardless of what's communicated.
...
\subsection{Experiment 1: Real-world Secure Protocols}

\subsection{Experiment 2: Random Protocols}
